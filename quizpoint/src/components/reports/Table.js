...
export default function QuizTable({ quizId, type, students }) {
    ...
    const [quizCode, setQuizCode] = useState(quizId)
    let columns = [
        {
    ...
    useEffect(() => {
        ...
        if (type === 'completion') {
            ...
            let quizRef = ref(db, `/schools/hvhs/quizzes/${quizCode}`)
                        ...
                        for await (let [key, value] of Object.entries(students)) {
                            ...
                            onValue(pathRef, (snapshot) => {
                                ...
                                if (snapshot.val() === null || undefined) {
                                    ...
                                    if (snapshot.val().quizzes.turnedin[quizCode]) {
                                        console.log('turned in' + dataForUser.name)
                                        let route = snapshot.val().quizzes.turnedin[quizCode]
                                        ...
                                        console.log('turned in' + dataForUser.name)
                                        let route = snapshot.val().quizzes.active[quizCode]
                                        if (!route) {
